...
def timed_layer(layer_func):
    def wrapper(*args, **kwargs):
        start_time = time.time()
        result = layer_func(*args, **kwargs)
        end_time = time.time()
        print(f"Layer: {layer_func.__name__}, Execution Time: {round((end_time - start_time)*1000,ndigits=4)} miliseconds")
        return result
    return wrapper



def measure_speed(model, input_shapes, num_runs=10):
    # Generate random data for each input shape
    input_data = [tf.random.normal(shape) for shape in input_shapes]

    start_time = time.time()
    for _ in range(num_runs):
        _ = model([input_data[0], input_data[1], input_data[2], input_data[3], input_data[4]])
    end_time = time.time()
...
#     # Depthwise Convolution
#     c = DepthwiseConv2D(kernel_size=kernelsize, strides=stride, padding=pad, use_bias=False, name='dw_convTransp' + number)(entrylayer)
#     c = BatchNormalization(fused=True, name='BN_dw' + number)(c)
#     c = Activation(tf.keras.activations.swish)(c)

#     # Pointwise Convolution
#     c = Conv2DTranspose(filters=channels, kernel_size=(1, 1), use_bias=False, name='pw_convTransp' + number)(c)
#     c = BatchNormalization(fused=True, name='BN_pw' + number)(c)
#     c = Activation(tf.keras.activations.swish)(c)
#     return c
...
def Img2BEV(img):
    ...
    # Function to replace Conv2D with DepthwiseConv2D followed by 1x1 Conv2D
...
def CrossAtt(prim,sec,number = '0'):
    ...
    # Self-Attention
    Query= DepthwiseConv2D((1,1), activation = 'linear', use_bias = False, name = 'SA/Query'+ number)(sec)
    Key= DepthwiseConv2D((1,1), activation = 'linear', use_bias = False, name = 'SA/Key'+ number)(prim)
    Value= DepthwiseConv2D((1,1), activation = 'linear', use_bias = False, name = 'SA/Value'+ number)(prim)
    ...
    out = out * Value
    out = DepthwiseConv2D((1,1), activation = 'linear', use_bias = False, name = 'SA/out'+ number)(out)
...
def My_Model(input_pillar_l, input_pillar_pos_l,input_pillar_r, input_pillar_pos_r,  input_img):
    ...
    conc = Concatenate()([c_img2,c_lidar2]) #384
    ...
    #c_radar3 = SmallBlock(c_radar1, nb_channels,(3,3),(2,2), number='c_Radar3')
    CrAtt_rad = CrossAtt(c_radar2,c_lidar2,number = '1_Radar_LIDAR')
    add_rad = Add(name = 'Add_CrAtt_FRadar1')([CrAtt_rad,c_lidar2])#64
    Add_rad_up = SmallBlock_up(add_rad, nb_channels*2,(3,3),(2,2),pad='same', number='up_1_Radar')
    ...
    return out
def measure_layer_speed(model, input_shapes):
    # Generate random data for each input shape
    input_data = [tf.random.normal(shape) for shape in input_shapes]
    times = []

    # Create a model for each layer and measure the execution time
    for i in range(len(model.layers)):
        sub_model = tf.keras.Model(inputs=model.inputs, outputs=model.layers[i].output)
        start_time = time.time()
        _ = sub_model([[input_data[0], input_data[1], input_data[2], input_data[3], input_data[4]]])
        end_time = time.time()
        layer_time = end_time - start_time
        times.append((model.layers[i].name, layer_time))

    # Print the time taken by each layer
    for layer_name, layer_time in times:
        print(f"Layer {layer_name}: \t{layer_time*1000:.2f} miliseconds")
...
if __name__ == '__main__':
    ...
    for i in range(1):
        measure_layer_speed(model, input_shapes)
        average_time = measure_speed(model, input_shapes)
        print(f"Average inference time per run: {average_time} seconds")
    ...
    #     # Set up TensorBoard callback for profiling
    #     log_dir = "logs/profile"  # Specify the log directory
        
    #     tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1, profile_batch='10,20')
    #     # Run a single prediction to profile
    #     model.predict(
    #         [input_pillar_l, input_pillar_pos_l, input_pillar_r, input_pillar_pos_r, input_img],
    #         callbacks=[tensorboard_callback]
    #     )

    #     # Create the Timeline object and write to a file
    #     # tl = timeline.Timeline(run_metadata.step_stats)
    #     # ctf = tl.generate_chrome_trace_format()
    #     with open('model_timeline.json', 'w') as f:
    #         f.write(ctf)
